...
def count_message(rules, messages):
    total = 0
...
with open("input.txt", 'r') as inputFile:
    raw_rules, message = inputFile.read().split('\n\n')
    raw_rules = raw_rules.splitlines()
    message = message.splitlines()

rules = rule_maker(raw_rules)
print(f"Part 1: {count_message(rules, message)}")
rules[8] = [[42], [42, 8]]
rules[11] = [42, 31], [42, 11, 31]
print(f"Part 2: {count_message(rules, message)}")
